...
while True:
	...
	if returncode == 0:
		...
		print (Fore.CYAN + "Looks like this router is already configured for WP..")
		output = command_noalert('10.255.0.20','yph546DKDUmsyCEJ','file print')
		if b'changelog_v1.1' in output:
	...
	process = subprocess.Popen(['ping','-c' ,'1', '10.255.0.29'], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
	stdout, stderr = process.communicate()
	returncode = process.returncode
	if returncode == 0:
		print (Fore.GREEN + "10.255.0.29 online")
		print (Fore.CYAN + "Looks like this router is already configured for WIFI Hub..")
		output = command_noalert('10.255.0.29','yph546DKDUmsyCEJ','file print')
		if b'changelog_v1.1' in output:
			print("Currently on v1.1")
			print ('\a')
			time.sleep(1)
		...
		if user_input.lower() == 'y':
			print(Fore.CYAN + 'Resetting the router!')
			output = command('10.255.0.29','yph546DKDUmsyCEJ','system reset-configuration')
			time.sleep(10)
			os.execl(sys.executable, sys.executable, *sys.argv)
		elif user_input.lower() == 'n':
			print(Fore.CYAN + 'Plug in a different router and start again')
			...
			print(Fore.CYAN + 'Type y or n')
		break
	print(Fore.RED + "10.255.0.29 not responding...")
...
output = command('10.255.0.29','','user set admin password=yph546DKDUmsyCEJ')


print ("******************Final Checks******************")
if configoutput.decode('utf-8') == "":
	print (Fore.GREEN + "No error while loading configuration")
else:
	print (Fore.RED + "Configuration Errors:"+ configoutput.decode('utf-8'))
	quit()

output = command_noalert('10.255.0.29','yph546DKDUmsyCEJ','system package print')
if b'routeros  7.8' in output:
        print(Fore.GREEN + "RouterOS V7.8")
else:
	print (Fore.RED + "RouterOS needs an update")
	quit()

output = command_noalert('10.255.0.29','yph546DKDUmsyCEJ','system routerboard print')
if b'current-firmware: 7.8' in output:
...
output = command_noalert('10.255.0.29','yph546DKDUmsyCEJ','interface wireless print')
if b'ssid="Activ8me Hotspot"' in output:
        print (Fore.GREEN + "SSID = Activ8me Hotspot")
else:
	print (Fore.RED + "SSID is incorrect!!")
	quit()

output = command_noalert('10.255.0.29','yph546DKDUmsyCEJ','interface vlan print')
...
output = command_noalert('10.255.0.29','yph546DKDUmsyCEJ','ip address print')
if b'10.255.0.29/24' in output:
        print (Fore.GREEN + "10.255.0.29 Configured")
else:
