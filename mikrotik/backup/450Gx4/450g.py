...
while True:
	...
	if returncode == 0:
		...
		if user_input.lower() == 'y':
			...
			print(Fore.CYAN + (Fore.CYAN + 'Plug in a different router and start again')
			time.sleep(10)
			os.execl(sys.executable, sys.executable, *sys.argv)
		else:
...
#Ping to see if hardware is ready to connect
#print(Fore.GREEN + "Checking if Mikrotik is connected..")
#while True:
#        if os.system('ping 192.168.88.1 -c 1 ') == 0 or time.time() > timeout:
#                print(Fore.GREEN + "Hello we're online... let's get to work.")
#                break
...
output = command('10.255.0.5','','/interface bridge port remove 0; /interface bridge port remove 1; /interface bridge port remove 2; /interface bridge port remove 3; /interface bridge remove bridge; /ip address remove 0; /interface bridge port add bridge=bridge-vlan500 interface=ether4; /interface bridge port add bridge=bridge-vlan501 interface=eth1-vlan501; /interface bridge port add bridge=bridge-vlan501 interface=eth5-vlan501; /interface bridge port add bridge=bridge-vlan521 interface=ether1 hw=no; /interface bridge port add bridge=bridge-vlan521 interface=ether2 hw=no; /interface bridge port add bridge=bridge-vlan521 interface=eth1-vlan521; /interface bridge port add bridge=bridge-vlan522 interface=eth1-vlan522; /interface bridge port add bridge=bridge-vlan522 interface=ether3 hw=no; /ip dns set servers=8.8.8.8,8.8.4.4; /ip route add distance=1 gateway=10.255.0.1; /ip service set telnet disabled=yes; /ip service set ftp disabled=yes; /ip service set www-ssl disabled=no; /ip service set www disabled=no; /ip service set ssh address=10.255.0.0/24; /ip service set api disabled=yes; /ip service set winbox address=10.255.0.0/24; /ip service set api-ssl disabled=yes; /system clock set time-zone-name=Australia/Melbourne; /user set admin password=yph546DKDUmsyCEJ; /system identity set name=450Gx4; /system ntp client set enabled=yes server=10.255.0.1,139.180.160.82; /system routerboard settings set protected-routerboot=disabled; /tool mac-server set allowed-interface-list=all; /tool mac-server mac-winbox set allowed-interface-list=all; /interface ethernet poe set ether5 poe-out forced-on;')
print ("Error Log:"+ output.decode('utf-8'))
time.sleep(2)
