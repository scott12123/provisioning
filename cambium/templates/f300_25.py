...
devicename = "SHORTNAME_F300-25SM_EXTXXXX_IP_Y_Z" #Edit this for each site
snmp_trap_community = "SNMP_SITENAME" #Edit for each site

def Restart_Configuration():
    with open('provisioningpi/cambium/rcp-SHORTNAME-f300-25-remote.py') as file:
         exec(file.read())
############Edit this bit only#################

os.system("sudo ip address add 192.168.0.1/24 dev eth0 > /dev/null 2>&1") #Add IP for connectivity to default IP 192.168.0.2

starting1 = time.time() #Lets see how long this takes?
functions.log(f"Starting SHORTNAME_F300-25 script...","INFO")
...
if gsheet.check_device_number_exists(number,devicename):
    while True:  # Keep asking until a valid response is given
        ...
        if user_choice == 'y':
            functions.log(f"Duplicate EXT{formatted_devicename} detected - Proceeding","INFO")
            break  # Breaks out of the loop and continues with the rest of the script
        elif user_choice == 'n':
            functions.log(f"Duplicate EXT{formatted_devicename} detected - Cancelled","ERROR")
...
starting2 = time.time()
waited_seconds = (starting2 - starting1)
waited_seconds = round(waited_seconds,1)
functions.log(f"Starting configuration for {formatted_devicename} after {waited_seconds}s","INFO")
start = time.time()
...
while True:  # Outer loop to restart the process if necessary
    ...
    if '4.7.0.1' in firmware:
        ...
        functions.log(f"Firmware already v4.7.0.1 for {formatted_devicename} - {serial_number}","INFO")
        ...
        while True:
            ...
            if current_status:
                if current_status == "0":
                    ...
                    functions.log(f"Firmware Upgraded for {formatted_devicename} - {serial_number}","INFO")
...
dest = f"/mnt/usbstick/rcp3/RCP3-{site}-Force300-25_{number}_{ip_address}_{current_time}.json"
...
functions.log(f"Created config file at {custom_config}","INFO")
...
while True:
    ...
    while True:
        ...
        if status_zero_count > 8:
            ...
            returncode = process.returncode
            if returncode == 0:
                functions.log(f"Configuration upload failed but ping responded on {ip_address} - {formatted_devicename}","ERROR")
    ...
    if 'rebooting' in status_message:
        functions.log(f"Configuration uploaded successfully - {formatted_devicename}","INFO")
...
if snmp_trap_community in snmptrap:
      functions.log(f"SNMP Trap Community 1 is correct - {formatted_devicename} - Reported: {snmptrap}","SNMP_ERROR")
else:
      functions.log(f"SNMP Trap Community 1 is wrong - {formatted_devicename} - Reported: {snmptrap} - Applying again!","SNMP_ERROR")
      print("SNMP Trap is wrong, reapplying trap config and saving")
      os.system(f"snmpset -v 2c -c SNMP-RW-ACT1v8me {ip_address} .1.3.6.1.4.1.17713.21.3.5.6.0 s {snmp_trap_community} .1.3.6.1.4.1.17713.21.4.3.0 i 1")
      time.sleep(10)
      print("Checking SNMP Trap again...")
#running this twice to see if it gives error both times
snmptrap = functions.snmpgeta8(ip_address,".1.3.6.1.4.1.17713.21.3.5.6.0")
if snmp_trap_community in snmptrap:
      print (Fore.GREEN + "SNMP Trap Community is correct")
      functions.log(f"SNMP Trap Community 2 is correct - {formatted_devicename} - Reported: {snmptrap}","SNMP_ERROR")
else:
      print (Fore.RED + "SNMP Trap Community is wrong!")
      functions.log(f"SNMP Trap Community 2 is wrong - {formatted_devicename} - Reported: {snmptrap}","SNMP_ERROR")
      quit()
...
if '1' in cnmenabled:
      ...
      quit()
functions.log(f"Configuration completed for {formatted_devicename}","INFO")
...
functions.log(f"Updated Gsheet with {formatted_devicename}","INFO")
...
minutes = seconds // 60
seconds %= 60
min_round = int(minutes)
sec_round = int(seconds)
print (Fore.GREEN + "Configuration Duration: %d:%02d:%02d" % (hour, minutes, seconds))
functions.log(f"{min_round}m {sec_round}s taken to configure {formatted_devicename}", "DURATION")
...
functions.log("Waiting for another F300 to be plugged in", "INFO")
