...
def Restart_Configuration():
    with open('cambium/nbn-f300-25-p2p.py') as file:
         exec(file.read())
...
while True:
    p2p_link = input("Enter 1 for master or 2 for slave: ")
    if p2p_link in ["1", "2"]:
        break
...
if p2p_link == "1":
    p2p = 'Master'
    source = '/mnt/usbstick/templates/NBN-Force300-25-P2P-Master-IP0-220-v1.0.json'
    dest = f"/mnt/usbstick/nbn/{device_name}-{ip_address}_{current_time}.txt"
    command = f"sudo cp {source} {dest} && sudo sed -i 's/10.255.0.220/{ip_address}/g' {dest} && sudo sed -i 's/F25_P2P_AP_IP0_220/{device_name}/g' {dest} && sudo sed -i 's/F25_P2P/{bridge_id}/g' {dest} && sudo sed -i 's/5745/{frequency}/g' {dest} "

else:
    p2p = 'Slave'
    source = '/mnt/usbstick/templates/NBN-Force300-25-P2P-Slave-IP0-221-v1.0.json'
    dest = f"/mnt/usbstick/nbn/{device_name}-{ip_address}_{current_time}.txt"
    command = f"sudo cp {source} {dest} && sudo sed -i 's/10.255.0.221/{ip_address}/g' {dest} && sudo sed -i 's/F25_P2P_SM_IP0_221/{device_name}/g' {dest} && sudo sed -i 's/F25_P2P/{bridge_id}/g' {dest}"
...
print_name = f"{p2p} {device_name}"
print(Fore.GREEN + "Printing label for P2P F300 radio")
