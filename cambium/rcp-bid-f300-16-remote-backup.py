...
provisioningpi/brother') #for label scripts
provisioningpi/common') #for common files/scripts
...
#Detected default F300-16, now ask for EXT number
...
if '4.7.0.1' in firmware: #SNMP should return value including 4.7.0.1
      ...
      while True:
           ...
           current_status = int_value.group()
...
print_v1.print_label(number,ip_address,"device")
print(Fore.GREEN + "Printing label for F300 box")
print_v1.print_label(number,ip_address,"box")
print(Fore.GREEN + f"Printing label for R195")
print_v1.print_label(number,ip_address,"device")
print(Fore.GREEN + f"Printing label for R195 packaging")
print_v1.print_label(number,ip_address,"device")

#reboot file storage pi
print(Fore.GREEN + "Reboot file storage pi")
...
try:
    ...
    # Executing the sudo reboot command
    # Using 'echo' to pass the password to sudo
    stdin, stdout, stderr = ssh.exec_command('echo {} | sudo -S reboot'.format(password))
    stdin.flush()

    # Reading output (if any)
...
gsheet.add_to_sheet(serial_number, mac_address, formatted_devicename, ip_address, fwv, hardware)
