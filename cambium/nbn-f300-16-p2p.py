...
def Restart_Configuration():
    with open('cambium/nbn-f300-16-p2p.py') as file:
         exec(file.read())
...
if p2p_link == "1":
    p2p = 'Master'
    source = '/mnt/usbstick/templates/NBN-Force300-16-P2P-Master-IP0-222-v1.0.json'
    dest = f"/mnt/usbstick/nbn/{device_name}-{ip_address}_{current_time}.txt"
    command = f"sudo cp {source} {dest} && sudo sed -i 's/10.255.0.222/{ip_address}/g' {dest} && sudo sed -i 's/F16_P2P_AP_IP0_222/{device_name}/g' {dest} && sudo sed -i 's/F16_P2P/{bridge_id}/g' {dest} && sudo sed -i 's/5745/{frequency}/g' {dest} "

else:
    p2p = 'Slave'
    source = '/mnt/usbstick/templates/NBN-Force300-16-P2P-Slave-IP0-223-v1.0.json'
    dest = f"/mnt/usbstick/nbn/{device_name}-{ip_address}_{current_time}.txt"
    command = f"sudo cp {source} {dest} && sudo sed -i 's/10.255.0.223/{ip_address}/g' {dest} && sudo sed -i 's/F16_P2P_SM_IP0_223/{device_name}/g' {dest} && sudo sed -i 's/F16_P2P/{bridge_id}/g' {dest}"
