...
def RCT_XV2_Generic_Configuration():
    with open('cambium/rcp3-xv2-2t0-spare-config.py') as file:
         exec(file.read())
...
device_name = "RCP3_SPARE_XV2_AP_0_213"
...
#source = '/mnt/usbstick/rcp3/RCP-Generic-XV2-IP-10.255.0.213.txt'
dest = f"/mnt/usbstick/rcp3/RCP-Generic-XV2-IP-10.255.0.213.txt"
#command = f"sudo cp {source} {dest} && sudo sed -i 's/10.255.0.213/{ip_address}/g' {dest} && sudo sed -i 's/GEN_XV2_AP8_IP3_80/{device_name}/g' {dest} && sudo sed -i 's/ssid \[Community\]_/ssid {community_name}_/g' {dest}"
