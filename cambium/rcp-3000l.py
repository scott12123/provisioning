...
def Restart_Configuration():
    with open('cambium/rcp-3000l.py') as file:
         exec(file.read())
...
bridge_id = input("Enter WifiBridge: ")
while True:
    try:
        frequency = float(input("Enter Frequency (4400–4840): "))
        if 4400 <= frequency <= 4840:
            break
        else:
            print("Frequency must be between 4400 and 4840.")
    except ValueError:
...
if antenna_type == "1":
    antenna = 'Omni'
    source = '/mnt/usbstick/templates/RCP3-ePMP3000L-Omni-IP10.255.0.210-v3.0.json'
    dest = f"/mnt/usbstick/rcp3/{device_name}_{current_time}.txt"
    command = f"sudo cp {source} {dest} && sudo sed -i 's/10.255.0.210/{ip_address}/g' {dest} && sudo sed -i 's/GEN_T1_3000L_OMNI_IP0_210/{device_name}/g' {dest} && sudo sed -i 's/5825/{frequency}/g' {dest} && sudo sed -i 's/WifiBridge_1/{bridge_id}/g' {dest}"

else:
    antenna = 'Sector'
    source = '/mnt/usbstick/templates/RCP3-ePMP3000L-Sector-IP10.255.0.210-v3.0.json'
    dest = f"/mnt/usbstick/rcp3/{device_name}_{current_time}.txt"
    command = f"sudo cp {source} {dest} && sudo sed -i 's/10.255.0.210/{ip_address}/g' {dest} && sudo sed -i 's/GEN_T1_3000L_SEC_IP0_210/{device_name}/g' {dest} && sudo sed -i 's/5825/{frequency}/g' {dest} && sudo sed -i 's/WifiBridge_1/{bridge_id}/g' {dest}"
