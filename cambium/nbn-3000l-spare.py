...
def Restart_Configuration():
    with open('cambium/nbn-3000l-spare.py') as file:
         exec(file.read())
...
print (Fore.GREEN + "Checking for factory default 3000L connected on 192.168.0.1")
while True:
...
password = functions.passwd('192.168.0.1')
...
#Hardware check - Make sure we're actually running a 3000L. We only check now in case the firmare is blocking SNMP access.
print (Fore.GREEN + "Checking Hardware")

stream = os.popen('snmpget -v 2c -c publicreadonly 192.168.0.1 .1.3.6.1.4.1.17713.21.1.1.2.0')
output = stream.read()

if '44' in output:
        print(Fore.GREEN + "Confirmed 5 GHz 3000L Radio")
        hardware = "3000L"
else:
        print(Fore.RED + "Not a 3000L")
        functions.log(f"Device is not a 3000L","ERROR")
        print(Fore.RED + "Check hardware model and that you're running the right config file")
        quit()

#Fetch the MSN (SN)
snsnmpd = os.popen('snmpget -v 2c -c publicreadonly 192.168.0.1 .1.3.6.1.4.1.17713.21.1.1.31.0')
snsnmp = snsnmpd.read()
serial_number = re.findall(r'"(.*?)"', snsnmp)[0]

#Fetch the ESN (MAC)
macsnmpd = os.popen('snmpget -v 2c -c publicreadonly 192.168.0.1 .1.3.6.1.4.1.17713.21.1.1.30.0')
macsnmp = macsnmpd.read()
mac_address = re.findall(r'"(.*?)"', macsnmp)[0]

#community_name = input("Enter Community Name: ")
while True:
    antenna_type = input("Enter 1 for omni or 2 for sector: ")
    if antenna_type in ["1", "2"]:
        break
    print("Invalid input. Please enter 1 or 2 only.")
device_name = "SPARE_3000L_T0_IP0_210"
ip_address = "10.255.0.210"
bridge_id = "WifiBridge_5"
frequency = "5765"
print("Configuring device as:")
print(device_name)
print(ip_address)
print(bridge_id)
print(frequency)
...
if antenna_type == "1":
    antenna = 'Omni'
    source = '/mnt/usbstick/nbn/nbn-ePMP3000L-Omni-IP10.255.0.210-v3.0.json'
    dest = f"/mnt/usbstick/nbn/{device_name}_{current_time}.txt"
    command = f"sudo cp {source} {dest} && sudo sed -i 's/10.255.0.210/{ip_address}/g' {dest} && sudo sed -i 's/3000L_Tx_OMNIx_SITENAME_IP/{device_name}/g' {dest} && sudo sed -i 's/SNMPCambiumNetworks/{device_name}/g' {dest} && sudo sed -i 's/5765/{frequency}/g' {dest} && sudo sed -i 's/WifiBridge_x/{bridge_id}/g' {dest}"

else:
    antenna = 'Sector'
    source = '/mnt/usbstick/nbn/nbn-ePMP3000L-Sector-IP10.255.0.210-v3.0.json'
    dest = f"/mnt/usbstick/nbn/{device_name}_{current_time}.txt"
    command = f"sudo cp {source} {dest} && sudo sed -i 's/10.255.0.210/{ip_address}/g' {dest} && sudo sed -i 's/3000L_Tx_SECx_SITENAME_IP/{device_name}/g' {dest} && sudo sed -i 's/SNMPCambiumNetworks/{device_name}/g' {dest} && sudo sed -i 's/5765/{frequency}/g' {dest} && sudo sed -i 's/WifiBridge_x/{bridge_id}/g' {dest}"
...
while True:  # Outer loop to restart the process if necessary
    ...
    firmware = functions.snmpget("192.168.0.1", ".1.3.6.1.4.1.17713.21.1.1.1.0")
    ...
    if '4.7.0.1' in firmware:
        ...
        setfirmware = os.system(f"snmpset -v 2c -c privatereadwrite 192.168.0.1 .1.3.6.1.4.1.17713.21.4.7.0 s {firmware_file} > /dev/null 2>&1")

        last_status = None
        status_zero_count = 0

        while True:
            current_status_value = functions.snmpget("192.168.0.1", ".1.3.6.1.4.1.17713.21.4.8.0")
...
print (Fore.GREEN + "3000L back online")


print (Fore.GREEN + f"Uploading {custom_config}")

while True:
    importjsonconfig = os.system(f"snmpset -v 2c -c privatereadwrite 192.168.0.1 .1.3.6.1.4.1.17713.21.6.4.1.0 s {custom_config} .1.3.6.1.4.1.17713.21.6.4.4.0 i 1  > /dev/null 2>&1")
    last_status = None
    status_zero_count = 0  # Counter for the number of times status is 0

    while True:
        current_status_value = functions.snmpget("192.168.0.1", ".1.3.6.1.4.1.17713.21.6.4.2.0")
        int_value = re.search(r'\d+$', current_status_value)
        current_status = int_value.group() if int_value else None
...
print (Fore.GREEN + f"3000L back online at {ip_address}.")
time.sleep(5)

#Print labels for the devices
current_date = datetime.now()
date = current_date.strftime("%d/%m/%Y")
print_name = f"{antenna} {device_name}"
print(Fore.GREEN + "Printing labels")
print_v1.pass_label(print_name,ip_address,f"PASS {date}")
print_v1.pass_label(print_name,ip_address,f"PASS {date}")
...
print ('Restart? Plug in another Cambium 3000L in with default IP')
print ('Or press Ctrl+Z to exit')
functions.log("Waiting for another 3000L to be plugged in", "INFO")
...
while True:
	...
	print(Fore.MAGENTA + "Plug another 3000L in or press ctrl+z to quit.... checking again")