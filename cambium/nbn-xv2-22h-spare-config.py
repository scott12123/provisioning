...
def RCT_XV2_Generic_Configuration():
    with open('/home/pi/provisioningpi/cambium/nbn-xv2-22h-spare-config.py') as file:
         exec(file.read())
...
community_name = "Spare"
device_name = "NBN_SPARE_XV2_AP_0_212"
ip_address = "10.255.0.212"
...
source = '/mnt/usbstick/nbn/nbn-Generic-XV2-22H-IP-10.255.0.212.txt'
dest = f"/mnt/usbstick/nbn/{device_name}-{ip_address}_{current_time}.txt"
command = f"sudo cp {source} {dest} && sudo sed -i 's/10.255.0.212/{ip_address}/g' {dest} && sudo sed -i 's/GEN_XV2-22H_SPARE_IPX_XXX/{device_name}/g' {dest} && sudo sed -i 's/ssid \[Community\]_/ssid {community_name}_/g' {dest}"
