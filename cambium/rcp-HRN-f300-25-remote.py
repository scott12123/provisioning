...
config_file = '/mnt/usbstick/templates/RCP3-Generic-Force300-25-IP-10.255.0.216-v2.0.json' #Shouldn't need changing
...
devicename = f"{abbreviated_site_name}_F300-25SM_EXTXXXX_IP_Y_Z" #Edit this for each site
snmp_trap_community = f"SNMP_{full_site_name}" #Edit for each site

site = abbreviated_site_name

def Restart_Configuration():
    with open(f'cambium/rcp-{site}-f300-25-remote.py') as file:
         exec(file.read())
...
functions.log(f"Starting {site}_F300-25 script...","INFO")
...
command = f"sudo cp {config_file} {dest} && sudo sed -i 's/{config_file_ip}/{ip_address}/g' {dest} && sudo sed -i 's/GEN_300-25_EXTxxxx_IP_x_x/{formatted_devicename}/g' {dest} && sudo sed -i 's/SNMP_SiteName/{snmp_trap_community}/g' {dest}"
