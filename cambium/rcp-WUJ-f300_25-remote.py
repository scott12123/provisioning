...
def Restart_Configuration():
    with open('rcp-WUJ-f300-25-remote.py') as file:
         exec(file.read())
############Edit this bit only#################
...
functions.log(f"Starting WUJ_F300-25 script...","INFO")
...
if snmp_trap_community in snmptrap:
      functions.log(f"SNMP Trap Community 1 is correct - {formatted_devicename} - Reported: {snmptrap}","INFO")
else:
...
if snmp_trap_community in snmptrap:
      ...
      functions.log(f"SNMP Trap Community 2 is correct - {formatted_devicename} - Reported: {snmptrap}","INFO")
else:
