...
devicename = "BUR_F300-25SM_EXTXXXX_IP_Y_Z" #Edit this for each site
snmp_trap_community = "SNMP_Burringurrah" #Edit for each site

def Restart_Configuration():
    with open('provisioningpi/cambium/rcp-bur-f300-25-remote.py') as file:
         exec(file.read())
############Edit this bit only#################
...
functions.log(f"Starting BUR_F300-25 script...","INFO")
...
if snmp_trap_community in snmptrap:
      ...
      functions.log(f"SNMP Trap Community 1 is wrong - {formatted_devicename} - Reported: {snmptrap}","SNMP_ERROR")
