...
devicename = "GEN_F300-25SM_XXXX_IP_Y_Z" #Edit this for each site
#snmp_trap_community = "SNMP_Generic" #Edit for each site
...
def f300_16(ext, temp_ip_address):
    print(f"Starting Spare #{ext}")
    site = devicename[:3]

    ip_address = f"10.255.0.216"
    formatted_devicename = devicename
...
#    print (Fore.GREEN + "Creating custom config")
    ...
    index = source.find("/usbstick/")
    dest_config = source[index + len("/usbstick/"):]
...
#        client.close()
    ...
    print (Fore.GREEN + f"Uploading Generic Config")
    while True:
...
#    print(Fore.GREEN + "Printing label for F300 radio")
    print_v1.print_label_switch("SPARE",ip_address,"device","GEN",mac_address)
#    print(Fore.GREEN + "Printing label for F300 box")
#    print_v1.print_label_switch("SPARE",ip_address,"box","GEN",mac_address)
...
# Input for quantity of devices to be configured
ap_input = int(input("Enter quantity of generic F300-25 required: "))
...
def run_process(ap, ip_queue):
    ...
    devicename = "GEN_F300-25SM_APXXXX_IP_Y_Z"
