...
source = '/mnt/usbstick/templates/RCP3-Generic-Force300-25-IP-10.255.0.216-v2.0.json'
dest = f"/mnt/usbstick/universal/universal_F30025_{current_time}.txt"
command = f"sudo cp {source} {dest} && sudo sed -i 's/SNMP-RO-ACT1v8me/CdZbUUcp3f/g' {dest} && sudo sed -i 's/SNMP-RW-ACT1v8me/cNmpF9wusE/g' {dest} && sudo sed -i 's/GEN_300-25_EXTxxxx_IP_x_x/{device_name}/g' {dest} && sudo sed -i 's/SNMP_Sitename/{community_name}/g' {dest}"
